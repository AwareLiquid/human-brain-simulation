"""vgg_snn_v2.py — VGG SNN + 数据增强 (RandomCrop + RandomFlip), 挑战 85%+。"""
import pickle
import tarfile
import numpy as np
import torch
...
def augment(x):
    """RandomCrop(32, padding=4) + RandomHorizontalFlip (向量化, batch级 crop)。"""
    B = x.shape[0]
    padded = F.pad(x, (4, 4, 4, 4), mode='reflect')   # (B, 3, 40, 40)
    top = torch.randint(0, 9, (1,)).item()
    left = torch.randint(0, 9, (1,)).item()
    out = padded[:, :, top:top+32, left:left+32]      # 向量化 crop
...
def main():
    ...
    for T in [5, 10]:
        print(f"=== VGG SNN + 数据增强 (T={T}, 40 epochs) ===")
        model = VGGSNN(T=T).to(device)
        train(model, Xtr, ytr, 40, 128, 1e-3)
        acc = evaluate(model, Xte, yte)
        print(f"VGG SNN + aug T={T}: {acc:.4f}")
